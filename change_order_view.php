...
if ($changeOrder['approved_by']) {
    $approver = getDB()->fetchOne("SELECT full_name FROM users WHERE id = ?", [$changeOrder['approved_by']]);
...
<div class="row mb-3">
    <div class="col">
        ...
        ?>">
            ...
            <?php if ($changeOrder['approved_by'] && $approverName): ?>
                <br><small>by <?php echo htmlspecialchars($approverName); ?> at <?php echo date('m/d/Y g:i A', strtotime($changeOrder['approved_at'])); ?></small>
            <?php endif; ?>
        </div>
    </div>
</div>
