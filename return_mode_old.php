    ...
    try {
        if ($_POST['action'] === 'start_return') {
            ...
            if ($pullsheet && $pullsheet['status'] === 'picked') {
                $_SESSION['return_session'] = ['type' => 'pullsheet', 'id' => $pullsheet['id'], 'returner_name' => $_POST['returner_name'], 'items' => []];
                foreach (getPullsheetItems($pullsheet['id']) as $item) {
                    $_SESSION['return_session']['items'][$item['item_id']] = ['name' => $item['item_name'], 'barcode' => $item['item_barcode'], 'needed' => $item['quantity_picked'], 'scanned' => 0];
                }
                echo json_encode(['success' => true]);
                exit;
            }
            echo json_encode(['success' => false, 'message' => 'Not found or not picked yet']);
            exit;
        }
        if ($_POST['action'] === 'scan_item') {
            ...
            if ($item && isset($_SESSION['return_session']['items'][$item['id']])) {
                ...
                echo json_encode(['success' => true, 'item' => $_SESSION['return_session']['items'][$item['id']], 'itemId' => $item['id']]);
                exit;
            }
        ...
        if ($_POST['action'] === 'complete_return') {
            foreach ($_SESSION['return_session']['items'] as $itemId => $data) {
                ...
                getDB()->query("DELETE FROM item_allocations WHERE pullsheet_id = ? AND item_id = ?", [$_SESSION['return_session']['id'], $itemId]);
            }
            getDB()->query("UPDATE pullsheets SET status = 'completed' WHERE id = ?", [$_SESSION['return_session']['id']]);
...
?>
<!DOCTYPE html>
<html><head><meta charset="UTF-8"><title>Return Mode</title>
...
<button class="btn btn-success btn-lg w-100" id="startBtn">Start Return</button>
</div></div></div></div>
<?php else: ?>
<div class="mb-4"><input type="text" class="form-control large-input" id="itemScan" placeholder="Scan item..." autofocus></div>
<div class="row">
<?php foreach ($returnSession['items'] as $itemId => $item): $s = $item['scanned'] == $item['needed'] ? 'complete' : 'incomplete'; ?>
...
<button class="btn btn-success btn-lg w-100" id="completeBtn">Complete Return</button>
<?php endif; ?>
</div>
...
const post=(a,cb)=>fetch('return_mode.php',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:a}).then(r=>r.json()).then(cb);
<?php if (!$returnSession): ?>
